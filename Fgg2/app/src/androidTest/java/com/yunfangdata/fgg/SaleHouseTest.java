 ...
 * 卖房估值
 */
public class SaleHouseTest extends ActivityInstrumentationTestCase2<SplashActivity> {

    private Solo solo;
    private int screenWidth;
    private int screenHeight;

    public SaleHouseTest() {
     ...
     * @throws Exception
     */
    public void testSaleHouse() throws Exception {

        //初始化主屏幕
        initHomeActivity(solo);

        //如果出现  卖房估值 -- 点他
        if (solo.searchText("卖房估值")) {
            solo.clickOnText("卖房估值");
        }

        //当前是不是   SaleHouseActivity
        solo.assertCurrentActivity("Expected SaleHouseActivity activity", "SaleHouseActivity");

        //另外一种输入的方法  小区名称
        solo.enterText(0, "五栋大楼");
        //另外一种输入的方法  面积
        solo.enterText(1, "2333");
        ...
        //另外一种输入的方法  预售总价
        solo.enterText(2, "2000");

        //另外一种输入的方法  楼栋
        solo.enterText(3, "发财大楼");
        //另外一种输入的方法  门牌号
        solo.enterText(4, "4栋404");
        ...
        //另外一种输入的方法  建成年代
        solo.enterText(7, "2015");
        //另外一种输入的方法  特殊因素
        solo.enterText(8, "我长得帅");
        ...
        boolean noteFound = solo.waitForText("卖房估值");
        //Assert that Note 1 test is found
        assertTrue("卖房估值 is not found", noteFound);
