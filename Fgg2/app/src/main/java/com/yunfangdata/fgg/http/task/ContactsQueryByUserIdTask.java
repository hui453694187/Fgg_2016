 ...
 * 查询某个用户的所有联系人.
 */
public class ContactsQueryByUserIdTask implements IRequestTask {
     ...
     */
    @Override
    public ResultInfo<ArrayList<PersonContacts>> getResponseData() {
        ...
        try {
            if (this.mData != null) {
                ...
                if (json.getString("success").equals("true")) {
                    ...
                    ArrayList<PersonContacts> data = new ArrayList<>();
                    for (int i = 0; i < arr.length(); i++) {
                        JSONObject temp = (JSONObject) arr.get(i);
                        data.add(new PersonContacts(temp));
                    }
                    result.Data = data;
                    result.Success = true;
     ...
     * @return 结果对象
     */
    public ResultInfo<ArrayList<PersonContacts>> request(String userName) {
        ResultInfo<ArrayList<PersonContacts>> result = new ResultInfo<>();
        String url = Constant.HTTP_ALL + Constant.HTTP_CONTACTSQUERYBYUSERID;
