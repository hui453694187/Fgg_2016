...
public class CleanEditText extends EditText {
	...
	public CleanEditText(Context context, AttributeSet attrs) {
		...
		this.context=context;
		init();
		//context.obtainStyledAttributes(attrs, R.styleable.CleanEditText)
	}
	...
	private void init(TypedArray ta) {
		...
		if (delImg == null) {
			delImg = ta.getDrawable(R.styleable.CleanEditText_drawable_icon);
		}
		// 设置图片大边框
	 ...
	 * @author kevin
	 * @date 2015-9-23 下午4:08:40
	 * @Description: 初始化 默认图标
	 */
	private void init() {
		// 获取输入框右边的图片
		delImg = getCompoundDrawables()[2];
		if (delImg == null) {
			delImg = getResources().getDrawable(R.drawable.check_choose);
		}
		// 设置图片大边框
		DensityHelper dh= DensityHelper.getInstance();
		int pxBound=dh.dip2px(this.context,iconSize);
